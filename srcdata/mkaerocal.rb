...
def addRecordData(loc, unitRec, from, to)
    ...
    @aerodromes[loc] = { 'dates' => [] } if !@aerodromes[loc]
...
#========================================================================
if ARGV.length == 0
    puts "usage: ruby mkaerocal.rb squadronFile1.xml [...]"
    exit -1
end
...
gDoc.elements.each('gazetteer/location') { 
    |loc|
    @aerodromes[cvt.iconv(loc.attributes['name'])] = { 'dates' => [] }
}
...
ARGV.each {
    ...
    jDoc.elements.each('squadrons/squadron') {
        |sq|
        ...
        sq.elements.each('location') {
            |loc|
            locName  = cvt.iconv(loc.attributes['name'])
